		...
		if (value === 0) {
			...
			if (SSGameObject.cells[i].possibleValues.length === 1) {
				...
				SSGameObject = JSON.parse(
					JSON.stringify(
						updater(0, SSGameObject, i, value, undefined)
					)
				);
			}
		}
...
const stratergyExclusiveCells = (SECGameObject) => {
	...
	for (let i = 1; i < 10; i++) {
		for (let j = 0; j < 9; j++) {
			...
			for (let k = 0; k < 9; k++) {
				possibleValues = [
					...SECGameObject.cells[cellsId[k]].possibleValues,
				];
				value = SECGameObject.cells[cellsId[k]].value;
				if (possibleValues.includes(i) && value === 0) {
					inIt.push(
						JSON.parse(
							JSON.stringify(SECGameObject.cells[cellsId[k]])
						)
					);
				}
			}
			...
			if (inIt.length !== 0) {
				a = inIt.reduce(
					(acc, x) => inIt[0].column === x.column && acc,
					true
				);
			}
			if (a === true) {
				//console.log(inIt,i)
				SECArray = SECGameObject.columns[
					inIt[0].column
				].unknownCellsId.filter(
					(x) =>
	...
	const repeat = (x) => {
		for (let i = 1; i < 10; i++) {
			for (let j = 0; j < 9; j++) {
				...
				for (let k = 0; k < 9; k++) {
					possibleValues = [
						...ASgameObject.cells[cellsId[k]].possibleValues,
					];
					value = ASgameObject.cells[cellsId[k]].value;
					if (possibleValues.includes(i) && value === 0) {
						inIt.push(
							JSON.parse(
								JSON.stringify(ASgameObject.cells[cellsId[k]])
							)
						);
					}
				}
				...
				if (inIt.length === 1) {
					...
					ASgameObject = JSON.parse(
						JSON.stringify(
							updater(0, ASgameObject, id, value, undefined)
						)
					);
					...
					const repeatTwo = (a, b) => {
						...
						if (ASgameArray.length !== ASIgnore.length) {
							ASgameArray = ASgameArray.filter(
								(x) => !ASIgnore.includes(x)
							);
							ASgameObject = JSON.parse(
								JSON.stringify(
									updater(
					...
					caseTwoInit = caseInit.filter(
						(x) => x.possibleValues.length === 2
					);
					caseThreeInit = caseInit.filter(
						(x) => x.possibleValues.length === 3
					);
					caseFourInit = caseInit.filter(
						(x) => x.possibleValues.length === 4
					);
	...
	if (UMode === 0) {
		...
		if (
			...
			UObject.columns[Ucolumn].unknownCellsId = [
				...UObject.columns[Ucolumn].unknownCellsId.filter(
					(x) => x !== UId
				),
			];
			...
			for (let i = 0; i < 9; i++) {
				...
				let z =
					UObject.cells[boxCells[i]].possibleValues.indexOf(UValue);
				if (z !== -1 && y !== UValue) {
			...
			for (let i = 0; i < 9; i++) {
				...
				let z =
					UObject.cells[rowCells[i]].possibleValues.indexOf(UValue);
				if (z !== -1 && y !== UValue) {
			...
			for (let i = 0; i < 9; i++) {
				...
				let z =
					UObject.cells[columnCells[i]].possibleValues.indexOf(
						UValue
					);
				if (z !== -1 && y !== UValue) {
